...
export async function handleUpload(request: Request, context: Context): Promise<Response> {
  ...
  if (request.method === 'GET' && pathParts.length === 4 && pathParts[3] === 'presigned') {
    const sessionId = pathParts[2];
...
async function uploadSessionFile(request: Request, context: Context, sessionId: string): Promise<Response> {
  // Authenticate
  ...
  // Save to R2
  try {
    await context.putR2Object(r2Key, body, contentLength ? parseInt(contentLength) : undefined);
...
async function getPresignedUrl(_request: Request, _context: Context, sessionId: string): Promise<Response> {
