...
class BleServiceClass {
  ...
  private async requestPermissions(): Promise<void> {
    if (Platform.OS === "android") {
      ...
      if (Platform.Version >= 35) {
        perms.push(
          PermissionsAndroid.PERMISSIONS.FOREGROUND_SERVICE_CONNECTED_DEVICE,
        );
      }
  ...
  private onDeviceDiscovered = (
    ...
    const isOBD2Adapter =
      deviceName.includes("obd") || deviceName.includes("elm");
  ...
  async connectToDevice(deviceId: string): Promise<Device> {
    ...
    try {
        ...
        if (service.uuid.toUpperCase() === targetServiceUUID) {
          ...
          for (const char of characteristics) {
            if (char.uuid.toUpperCase() === targetCharUUID) {
              const props =
                (char as { properties?: Record<string, boolean> }).properties ||
                {};
              if (props.write) {
      ...
      const service = services.find(
        (s) => s.uuid.toUpperCase() === targetServiceUUID,
      );
      if (service) {
  ...
  // Event emitter
  // Event emitter
  private listeners: Map<string, Set<(...args: any[]) => void>> = new Map(); // eslint-disable-line @typescript-eslint/no-explicit-any
